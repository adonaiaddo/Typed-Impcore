...
fun typeof (e , Delta , Gamma) = 
      let 
        fun literal (NUM x) = inttype
         |  literal (BOOLV x) = booltype
         |  literal (SYM x) = symtype 
         |  literal (NIL) = FORALL (["'a"] , listtype tvA)
         |  literal (PAIR (a, NIL)) =
                    listtype (typeof (LITERAL a, Delta , Gamma))
         | literal (PAIR (x , y)) = 
                  (case (typeof (LITERAL x, Delta, Gamma) ,
                         typeof (LITERAL y, Delta, Gamma))
                    of (t1, CONAPP (TYCON "list" , [t2])) =>
                      if eqType (t1, t2) then listtype t2
                      else raise TypeError "types do not match"
                    | _  => raise TypeError 
                         "second value must be a NIL or a PAIR" )
         | literal _   = raise TypeError  "literal not valid"
        

         fun  ty (LITERAL(x))  =  literal x
            | ty (VAR(x))      =  find (x, Gamma)
            | ty (SET(x, e))    =  
                   (case (ty e, find (x, Gamma))
                    of (t1, t2) =>
                    if eqType (t1, t2)
                     then t1
                    else
                    raise TypeError "t1 and t2 must be of same type")
                    ...
                    (case (ty e1 , ty e2, ty e3)
                        of (TYCON "bool" , t2, t3) =>
                            if eqType (t2, t3) then t2
                                ...
                                "e1 and e2 have different types"
                        | _ => raise TypeError "e1 should be of type bool") 
                                              
        
                                           
            | ty (WHILEX (e1, e2))      =
                (case (ty e1, ty e2)
                 of (TYCON "bool", t) => unittype
                   | _  => raise TypeError "e1 must be of type bool")
            | ty (BEGIN [])             = unittype
            | ty (BEGIN exps)           =
                     (case map ty exps
                       of (ts) => List.last ts)
             ...
             | ty (APPLY (e, exps))      =
                (case (ty e, map ty exps)
                 of (FUNTY (types, p), ts) =>
                   if eqTypes(types, ts)
                   then p
                 else raise TypeError "Must be equal types"
                | _ => raise TypeError "e must be a function")

            | ty (LETX  (LET, exps, e)) =
                 typeof (e, Delta, foldr (fn ((x, e1), Gamma) =>
                                    bind (x, ty e1, Gamma))
                                    Gamma 
                                    exps)
            ...
            | ty (LETRECX (tyxs, e)) =
                let
                   val checkLambdas =
                     List.all (fn ((v, t), e1) => case e1
                       of (LAMBDA (ys, y)) => true
                       | _   => false) tyxs

                   val newEnv =
                      foldr (fn (((v, t), e1), Gamma) =>
                          bind (v, t, Gamma))
                       Gamma
                       tyxs

                    val checkEqTypes =
                       List.all (fn ((v, t), e1) => eqType(t, typeof
                         (e1, Delta, newEnv))) tyxs

                    val types =
                       map (fn ((v, t), e1) => asType(t, Delta)) tyxs

                in
                    if checkLambdas andalso checkEqTypes
                    then typeof (e, Delta, newEnv)
                  else
                     raise TypeError "All should be Lambdas"
                end
            | ty (LAMBDA (xs, e)) =
               let
                 val types =
                     map (fn (v, t) => asType(t, Delta)) xs

                 val etype =
                    typeof (e, Delta, foldr (fn ((v, t), Gamma) => 
                                 bind (v, t, Gamma))
                                                Gamma
                                                xs)
               in
            ...
            | ty (TYLAMBDA (tyxs, e)) =
               let
                  val checknotfvar =
                    List.all (fn (tyxs) =>
                      not (member tyxs (freetyvarsGamma Gamma))) tyxs

                  val newdeltaEnv =
                     foldr (fn (tyx, newdeltaEnv) 
                            => bind (tyx, TYPE, newdeltaEnv))
                                             Delta
                                             tyxs
                in
                  if checknotfvar
                    then FORALL(tyxs, typeof(e, newdeltaEnv, Gamma))
                 else
                    raise TypeError "There is a free type variable"
                end
            | ty (TYAPPLY (e, tyexps)) =
               let
                 val types = map (fn (tyexp) => asType(tyexp, Delta)) tyexps

               in
                instantiate (ty e, tyexps, Delta)
               end
...
fun typdef (def , kind_env , Gamma) =
    let 
      fun tdef (VAL (x , e)) = 
              (case typeof (e , kind_env, Gamma)
                of (t1) => (bind (x , t1, Gamma), typeString t1))
              
              
      |   tdef (VALREC (x , t, e)) = (case e
                  of (LAMBDA (ys, y)) =>
                    let
                      val newEnvTy    = bind (x, asType(t, kind_env), Gamma)
                      val etype       = typeof (e, kind_env, newEnvTy)
                      val checkEqType = eqType(etype, t)
                    in
                      if checkEqType
                        then (newEnvTy, typeString etype)
                      else
                        raise TypeError "VALREC e and v must have the same type"
                    end
                  | _ => raise TypeError "VALREC should be lambda exp")
      |   tdef (EXP (e))    =    tdef (VAL ("it" ,e))
      |   tdef (DEFINE (f , t, lambda_e)) = 
                    (case lambda_e
                      of ( tyxs, e) =>
                       let
                         val types = map (fn (x, t) => t ) tyxs
                       in
                        tdef (VALREC(f, FUNTY(types, t), LAMBDA lambda_e))
                       end)
